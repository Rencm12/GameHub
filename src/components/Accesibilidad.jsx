...
const COPY = {
  es: {
    title: "Menu de accesibilidad",
    close: "Cerrar menu de accesibilidad",
    language: "Idioma",
    profile: "Perfil",
    reset: "Restablecer",
    levels: ["Desactivado", "Nivel 1", "Nivel 2", "Nivel 3", "Nivel 4"],
    languages: {
      es: "Espanol",
      en: "English",
      pt: "Portugues",
  ...
  en: {
    ...
    profile: "Profile",
    reset: "Reset",
    levels: ["Off", "Level 1", "Level 2", "Level 3", "Level 4"],
    languages: {
      es: "Spanish",
      en: "English",
      pt: "Portuguese",
  ...
  pt: {
    ...
    profile: "Perfil",
    reset: "Redefinir",
    levels: ["Desativado", "Nivel 1", "Nivel 2", "Nivel 3", "Nivel 4"],
    languages: {
      es: "Espanhol",
      en: "English",
      pt: "Portugues",
...
function savePreferences(preferences) {
  ...
  window.dispatchEvent(
    new CustomEvent("accessibility-language-change", {
      detail: { language: preferences.language },
    }),
  );
}

export default function AccessibilityMenu({ open = true, onClose = () => {} }) {
  ...
  const languageOptions = Object.keys(text.languages);
  const profileOptions = Object.keys(text.profiles);
  ...
  useEffect(() => {
    if (!settings.readingMask && settings.cursor === 0) {
      return undefined;
    }
  ...
  const toggleTool = (tool) => {
    ...
    setSettings((prev) => {
      if (tool.levels === 0) {
        return { ...prev, [tool.id]: !prev[tool.id] };
      }
  ...
  return (
    <>
      ...
      {open && (
        ...
        className="fixed top-24 right-4 z-[10000] w-[calc(100%-2rem)] max-w-sm bg-slate-100 rounded-xl overflow-hidden shadow-xl font-sans animate-fadeIn md:right-8"
        >
        ...
        <div className="p-4 space-y-1">
          <Dropdown
            label={text.language}
            value={text.languages[language]}
            options={languageOptions}
            getLabel={(option) => text.languages[option]}
            open={langOpen}
            onToggle={() => {
          ...
          <Dropdown
            label={text.profile}
            value={text.profiles[profile]}
            options={profileOptions}
            getLabel={(option) => text.profiles[option]}
            open={profileOpen}
            onToggle={() => {
          ...
          <div className="grid grid-cols-2 gap-3 pt-3">
            {TOOL_CONFIG.map((tool) => (
              <ToolCard
                key={tool.id}
                tool={tool}
                label={text.tools[tool.id]}
                active={isActive(tool)}
                level={settings[tool.id]}
                levelLabel={text.levels[settings[tool.id] || 0]}
          ...
          {Array.from({ length: levels }).map((_, index) => (
            <span
              ...
              className={`h-1.5 w-4 rounded-full ${
                index < level ? "bg-blue-900" : "bg-slate-200"
              }`}
            />
          ))}
        </div>
